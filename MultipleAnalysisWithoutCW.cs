...
{
    public partial class MultipleAnalysisWithoutCW : Form
    {
        ...
        public MultipleAnalysisWithoutCW()
        {
            InitializeComponent();
        }

        private void MultipleAnalysisWithoutCW_Load(object sender, EventArgs e)
        {
            SelectDocuments();
            LoadItems();
            LoadDictionaryItems();
            ...
            foreach (var match in matches)
            {
                ...
                foreach (var item in fullLoop)
                {
                    ...
                    if (fullLoop.Length == 1)
                    {
                        ...
                        {
                            Info[value]["OWN"][level][0]++;
                            Info[value]["OWN"][4][0]++;
                        }
                        else
                        {
                            Info[value]["OWN"][level][1]++;
                            Info[value]["OWN"][4][1]++;
                        }
                    ...
                    foreach (var innerItem in fullLoop)
                    {
                        ...
                        if (!Info[value].ContainsKey(innerItem.Substring(1)))
                        {
                            Info[value].Add
                                        (
                        ...
                        {
                            Info[value][innerItem.Substring(1)][level][0]++;
                            Info[value][innerItem.Substring(1)][4][0]++;
                        }
                        else
                        {
                            Info[value][innerItem.Substring(1)][level][1]++;
                            Info[value][innerItem.Substring(1)][4][1]++;
                        }
                    }
                }
            }
